...
import { useSolarLighting, mapOrbitalDistanceToVisual, mapBodySizeToVisual, mapOrbitalPositionToVisual, getMaterialConfig, getRotationAngle, createPlanetMesh, createSunMesh, getTrajectoryConfig, createOrbitTrajectory, disposeOrbitTrajectory, type BodyType, getPlanetMaterialConfig, calculateInitialEarthRotationAngle, getBodyOrbitalElements, simulationTimeToUTC, calculateSubsolarPoint, runGeoProjectionSelftest, GeographicLayer, EarthGeoLodMonitor, useEarthGeoLodDatasets, type GeoLodLevel, type CountriesAdm0Data } from "@/rendering"
  ...
  ;(globalThis as any).__diag = {
    get state() {
      return {
        ...
        target: controlsRef.current ? controlsRef.current.target.toArray().map(v => +v.toFixed(2)) : null,
    ...
    <group position={pos}>
      <group ref={rotationGroupRef}>
        ...
        {planet.id === 'earth' && geoLod1Data && geoLodLevel >= 1 && (
          <GeographicLayer earthRadius={visualRadius} data={geoLod1Data} lodLevel={1} />
        )}
        {planet.id === 'earth' && geoLod2Data && geoLodLevel >= 2 && (
          <GeographicLayer earthRadius={visualRadius} data={geoLod2Data} lodLevel={2} />
        )}
